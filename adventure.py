...
#This funtions prints out all messages when invoked.
...
def cave(item, option):
    if "sword" in item:
        ...
        print_pause("\nYour eye catches a glint of metal behind a "
                    "rock.")
        print_pause("\nYou have found the magical Spear of Zeus!")
...
def house(item, option):
    ...
    while True:
        playerChoice = input("Would you like to (1) fight or (2) "
                        "run away?")
        if playerChoice == "1":
            if "sward" in item:
...
def play_game():
    item = []
    option = random.choice([f"medusa", "pirate", "dinosaur", "troll", "wicked fairie"])
    intro(item, option)
    field(item, option)
